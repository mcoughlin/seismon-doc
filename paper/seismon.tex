...
\begin{figure}[t]
 ...
 \end{tikzpicture}
 \end{center}
 \caption{A flow chart of the \emph{Seismon} pipeline.}
 \label{fig:flowchart}
\end{figure}
...
\hline
Detector & K & $M$ & $r$ & $h$ & $\rm Rf$  \\ \hline
LHO \newline(USGS only) & -6.8 & 0.06 & $2.5 \times 10^{-7}$ & $2.1 \times 10^{-3}$ & $1.2 \times 10^{6}$ \\ \hline
LHO & -24.1 & 3.4 & $2.9 \times 10^{-8}$ & $2.7 \times 10^{-3}$ & $9.2 \times 10^{5}$ \\ \hline
LLO \newline(USGS only) & -6.6 & 0.28 & $2.8 \times 10^{-7}$ & $-1.5 \times 10^{-3}$ & $9.6 \times 10^{5}$ \\ \hline
LLO & -15.3 & 2.2 & $5.6 \times 10^{-8}$ & $-1.2 \times 10^{-3}$ & $4.2 \times 10^{5}$\\ \hline
\end{tabular}
\caption{Best fit parameters to the $\vec\theta$ coefficients from equation (\ref{eq:MLA}). \rednote{We need to explain in much more detail what this table shows. I personally don't understand any of the parameters in this table... Brief names of parameters should be given in this caption. Extensive explanation in the text. Also, we don't write what USGS only means.}}
\label{table:MLAfit}
\end{table}
...
Figure~\ref{fig:MLA} shows the efficiency curves using the earthquake parameter derived parameters \rednote{What does EQ parameter derived parameters mean?}. The curves that include the observed ground velocity at the sites are similar. In general, there is a trade-off between false-alarm probability and efficiency standard probability. The more false alarms one is willing to accept, the higher the rate of earthquakes that will result in lockloss is possible. \rednote{This sentence needs polishing. We don't influence the nature of EQs and detector response by doing our analyses...} For example, if we adopt a false-alarm probability threshold of 0.5, between 90 -- 100\% of earthquakes can be caught.
	
\begin{figure}[h]
 \includegraphics[width=3.5in]{lockloss_fap_errorbars.pdf}
 \centering
 \caption{Efficiency curves using both earthquake parameter derived parameters only as well as one which includes observed ground velocity at the sites. \rednote{Again, what does EQ parameter derived parameters mean?}}
 \label{fig:MLA}
\end{figure}
